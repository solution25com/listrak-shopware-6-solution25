...
class SyncCustomersCommand extends Command
{
    ...
    {
        $this->setDescription('Syncs customers to Listrak for all sales channels.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($salesChannelContexts as $salesChannelContext) {
            ...
            if (!$clientId || !$clientSecret) {
                $output->writeln('<info>Listrak customer sync has been skipped. The API keys are missing.</info>');
                return Command::FAILURE;
            }
            $this->messageBus->dispatch(
                new SyncCustomersMessage(0, 200, null, $salesChannelContext->getSalesChannelId())
            );
        }
        $output->writeln('<info>Listrak customer sync has been dispatched to the queue.</info>');
